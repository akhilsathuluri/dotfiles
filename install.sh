...
#
# macOS takes its tools from Homebrew instead of pinned release assets: brew
# resolves arm64 vs x86_64 itself and carries the system deps (libevent, ncurses)
# that the source builds here assume. The trade-off is that macOS versions are
# whatever brew currently ships - the pins below are the Linux contract only.
case "$(uname -s)" in
    Linux)
    ...
    Darwin)
        ...
        ;;
    *)
        echo "dotfiles supports Linux (Ubuntu 24.04 / Debian 13) and macOS. Detected: $(uname -s)." >&2
...
# brew_install <formula>... - install only the formulae that are missing.
brew_install() {
    local pkg missing=()
    for pkg in "$@"; do
    ...
    log "Installing brew formulae: ${missing[*]}"
...
install_ghostty() {
    is_linux || return 0 # macOS gets the cask in install_brew_packages
    if command -v ghostty &>/dev/null; then
        ok "Ghostty already installed"
        return
    fi
    if ! is_ubuntu; then
        warn "Skipping Ghostty: the PPA is Ubuntu-only - install it manually if needed"
        return
    fi
    log "Installing Ghostty..."
...
install_nodejs() {
    is_linux || return 0 # macOS gets node from brew
...
install_nvim_plugins() {
    ...
    # Prefer our pinned build; fall back to PATH so brew's nvim is found on macOS.
    local nvim_bin
    ...
    # macOS has no GNU timeout unless coreutils is on PATH; perl's alarm is portable.
    if command -v timeout &>/dev/null; then
        timeout 300 "$nvim_bin" --headless "+Lazy! install" "+Lazy! restore" +qa >/dev/null 2>&1 || true
    else
        perl -e 'alarm shift; exec @ARGV' 300 "$nvim_bin" --headless \
            "+Lazy! install" "+Lazy! restore" +qa >/dev/null 2>&1 || true
    fi
...
install_tmux() {
    is_linux || return 0 # macOS gets tmux from brew
...
gate_tools() {
    if is_macos; then
        install_brew
        brew_install git-cliff gitleaks ruff shellcheck shfmt go-task tmux
        return
    fi
    install_apt_packages
    install_git_cliff
...
all_tools() {
    # macOS: brew covers everything the pinned Linux tarballs below provide, so
    # only the platform-neutral steps (npm, git clone) are run after it.
    if is_macos; then
        install_brew
        install_brew_packages
        install_hunk
