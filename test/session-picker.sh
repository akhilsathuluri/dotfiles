...
# Either escape form: unthemed the shared file falls back to ANSI slots, and a
# `theme` switch replaces them with the palette's truecolor hex (asserted below).
list_icon=$("$PICKER" --list | grep -F api | grep -oE $'\033\[[0-9;]+m.' | head -1)
prev_icon=$("$PREVIEW" api | grep -F 'state:' | grep -oE $'\033\[[0-9;]+m.' | head -1)
