...
STUB=$(mktemp -d)
printf '#!/bin/sh\nexit 1\n' >"$STUB/pkill"
# A real stub, not just an absent tmux: apply_tmux returns early when `command -v
# tmux` fails, so a PATH without it would skip the file this suite asserts on.
# brew's tmux and the pinned ~/.local/bin one are both off /usr/bin:/bin.
printf '#!/bin/sh\nexit 0\n' >"$STUB/tmux"
chmod +x "$STUB/pkill" "$STUB/tmux"
